"""
Utilities for group verification functionality
"""
...
def analyze_results(results_df: pd.DataFrame) -> Tuple[int, int, int]:
    """Analyze verification results and return statistics."""
    total = len(results_df)
    verified = results_df["est_dans_groupe"].fillna(False).sum()
    failed = total - verified
    return total, int(verified), int(failed)

def format_verification_summary(results_df: pd.DataFrame) -> str:
    """Format a summary string of verification results."""
    total, verified, failed = analyze_results(results_df)
    return f"""
